package com.mee.sangsil.common.excel;
...
public class ExcelRead {
	public static List<Map<String, String>> read(ExcelReadOption excelReadOption) {
		...
		for (int i = 0; i < sheetNum; i++) {
			...
			int numOfRows = sheet.getPhysicalNumberOfRows(); // 유효한 데이터가 있는 행의
																// 개수를 가져온다.
			...
			for (int rowIndex = excelReadOption.getStartRow() - 1; rowIndex < numOfRows; rowIndex++) {
				...
				/*
				 * 워크북에서 가져온 시트에서 rowIndex에 해당하는 Row를 가져온다. 하나의 Row는 여러개의 Cell을
				 * 가진다.
				 */
				...
				if (row != null) {
					 ...
					 */
					numOfCells = row.getPhysicalNumberOfCells(); // 한개의 행마다 몇개의
																	// cell이 있는지
																	// 체크
