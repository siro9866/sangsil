...
<!-- <script src="/resources/js/plugIn/thumnail/jquery.nailthumb.1.1.min.js"></script> -->
<!-- <link rel="stylesheet" href="/resources/css/plugIn/thumnail/jquery.nailthumb.1.1.min.css"> -->
<style>
	.my-thumb-1 {
		float: left;
		width: 220px;
...
var updateUrl = "/admin/board/update.mee";
var insertUrl = "/admin/board/insert.mee";
...
function deleteFile(file_id){
	$.ajax({
		type:"post",
		async:true,
		url:"/admin/board/deleteFile.mee",
...
function updateFile(file_id, index){
	
	$.ajax({
		type:"post",
		async:true,
		url:"/admin/board/updateFile.mee",
		data: {"file_id":file_id, "disp_order":$("#file_disp_order_"+index).val()},
		dataType:"json",
		success:function(data){
			if(data.rCode == "0000"){
// 				alert("저장 되었습니다.");
...
<!-- 게시판구분 -->
<spring:eval expression="@config['CD_ID_BAA03']" var="prop_board_gbn" />
							...
							<div class="form-group">
								<c:choose>
									...
									<c:otherwise>
										...
										<c:forEach items="${fileList }" var="resultFile" varStatus="status">
											<div class="my-thumb-1">
												<img src="${resultFile.view_path_name}${resultFile.file_name}">
												<div>
													<label>정렬</label>
													<input type="text" name="file_disp_order" id="file_disp_order_${status.index }" maxlength="3" size="3" value='<c:out value="${resultFile.disp_order }" />'/>
													<button type="button" class="btn btn-sm btn-warning" onclick="updateFile(${resultFile.file_id},${status.index })"><i class="fa fa-save "></i>&nbsp;저장</button>
													<button type="button" class="btn btn-sm btn-danger" onclick="deleteFile(${resultFile.file_id})"><i class="fa fa-trash "></i>&nbsp;삭제</button>
												</div>
											</div>
